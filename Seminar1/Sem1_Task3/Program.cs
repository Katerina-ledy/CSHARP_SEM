...
Console.WriteLine("Введите трёхзначное целое положительное число");
...
if (num >= 100 && num <= 999)   // if (num > 99 && num < 1000)
{
    ...
    Console.WriteLine($"Сумма первой и последней цифры треёхзначного числа = {result}");
}
else
{
    Console.WriteLine("Ошибка! Число не является трёхзначным, целым или положительным!");
}
